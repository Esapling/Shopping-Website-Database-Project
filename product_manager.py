...
class Product(DatabaseManagement):
    def __init__(self) -> None:
        ...
        self.db_junction = DatabaseManagement(table_name="product_junction")

    def getCategoryProducts(self, category_id: int):
        product_ids = []
        #get products' ids which belong to the given category from the junction table
        with psycopg.connect(**self.db_params)as connection:
            with connection.cursor() as cur:
                query_junction = f"select product_id from product_junction where category_id=%s"
